...
function Notes() {
  ...
  const handleGroupClick = (index) => {
    setClickedGroup(clickedGroup === index ? null : index);
  };
      ...
      <div>
        ...
        >
          ...
          {groups.map((group, index) => (
            <div
              key={index}
              className="box-for-each-note"
              onClick={() => handleGroupClick(index)}
            >
              {/* Display box content if group is clicked */}
              {clickedGroup === index ? (
                <div>
                  <div className="titleofnote">
                    {/* Your box content for the clicked group */}
                    <p>{group.name}</p>
                    {/* ... (other content specific to the clicked group) */}
                  </div>
                </div>
              ) : null}
            </div>
...
{clickedGroup === null && (
          <div className="rightside">
            <img src={centerimage} className="centerimg" alt="Center Image" />
            <p className="pocket-notes"> Pocket Notes</p>
            <p className="text1">Send and receive messages without keeping your phone online.</p>
            <p className="text2">Use Pocket Notes on up to 4 linked devices and 1 mobile phone</p>
            <p className="text3">
              <img src={Vector} height="13px" width="13px" alt="Vector Icon" /> end-to-end encrypted
            </p>
          </div>
        )}
