...
function Notes() {
  ...
  const [rightSideClasses, setRightSideClasses] = useState(
    groups.map(() => "rightside")
  );

  useEffect(() => {
    const updatedClasses = [...rightSideClasses];
    inputVisibility.forEach((visible, index) => {
      updatedClasses[index] = visible ? "box-for-each-note" : "rightside";
    });
    setRightSideClasses(updatedClasses);
  }, [inputVisibility, groups]);
  ...
  return (
    <div>
      ...
      >
        ...
        <div>
          {groups.map((group, index) => (
            <div key={index} className={rightSideClasses[index]}>
          ...
          {!inputVisibility[index && <div className="rightside"><img src={centerimage} className="centerimg"></img>
