...
function formatDateTime(dateTime) {
  ...
  const time = dateParts[1].trim(); // Extract time part
  const [month, day, year] = dateParts[0].split(" "); // Extract month, day, year

  return `${time} ${day} ${month} ${year}`; // Construct the desired format
}
